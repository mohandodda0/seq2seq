...
def indexesFromSentence(lang, sentence):
    return [lang.word2index[word] if word in lang.word2index else lang.word2index['OOV'] for word in sentence.split(' ')]
...
def evaluate(encoder, decoder, question, answer, inputlang, outputlang, criterion, max_length=MAX_LENGTH):
    with torch.no_grad():
        input_tensor = tensorFromSentence(inputlang, question)
        target_tensor = tensorFromSentence(outputlang, answer)
        input_length = input_tensor.size()[0]
        encoder_hidden = encoder.initHidden()
...
def evaluated(encoder, decoder, inputlang, outputlang, pairss2):
    i = 0
    for pair in pairss2:
...
with open('data/jokes_cleaned_test.txt', 'r') as reader:
    l = []
    for line in reader:
        l += [line.replace(' <SEP> ','\t').replace(' <|endoftext|> ', '')]

with open ('data/q2-a2.txt', 'w') as writer:
    for i in l:
        if 'nsfw' not in i:
            writer.write(i)
